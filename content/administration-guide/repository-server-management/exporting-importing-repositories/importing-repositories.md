...
Ensure that the repositories of interest have been [exported](/administration-guide/repository-server-management/exporting-importing-repositories/exporting-repositories) to files, and that you have access to those files from your host.

To import repositories, perform the following:

1.  If not already running, [open](/administration-guide/repository-server-management/launching-the-repository-server-control-panel) the Repository Server Control Panel dialog box.
...
5.  In the Import Repository dialog box, you have the following options:
    ...
    -   To import repository files as replication repositories, see [Importing a repository as an replication](/administration-guide/repository-server-management/exporting-importing-repositories/importing-a-repository-as-a-replication#step_rbp_hbl_nm).
    -   To overwrite existing repositories, select the **Overwrite Repository** check box. Before overwriting the repositories,[stop the repository server](/administration-guide/repository-server-management/starting-and-stopping-the-repository-server/stopping-the-repository-server).
    ...
    -   To remove a repository file that was selected in step \#[3](/administration-guide/repository-server-management/exporting-importing-repositories/importing-repositories#step_sfg_gh2_nm), click **Remove** on the **Action** field.

        {{<tip>}} You can select multiple repository files for removal by pressing your keyboard's Shift or Control key.

    -   To import extra repository files, click the **Add** button, then repeat steps \#[3](/administration-guide/repository-server-management/exporting-importing-repositories/importing-repositories#step_sfg_gh2_nm) through \#[4](/administration-guide/repository-server-management/exporting-importing-repositories/importing-repositories#step_stp_gh2_nm).
6.  Click **OK**.
    ...
    {{<note>}} If the status of the imported repository files is Error, please refer to [Troubleshooting repository import](/administration-guide/repository-server-management/exporting-importing-repositories/troubleshooting-repository-import).
