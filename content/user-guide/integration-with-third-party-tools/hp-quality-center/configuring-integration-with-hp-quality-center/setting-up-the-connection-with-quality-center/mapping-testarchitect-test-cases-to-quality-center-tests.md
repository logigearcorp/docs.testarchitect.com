...
1.  In the TestArchitect explorer tree, right-click the node of a test module \(either [default](/user-guide/variations/default-variations) or non-default [variation](/user-guide/tests/creating-a-test-module-variation)\) containing the test cases to be mapped, or a test folder, or the **Tests** node itself, and then click **External Tool** \> **HP ALM** \> **Map Test Cases**.
