...
![](/images//Images/Listview_data_set.png)

The following operations can be performed on a data set list view:

## Common operations

You can find various common operations that can be performed on data set list views at [Common operations](/TA_Help/Topics/Listview_common_operations.html).
...
Data sets can be exported to archive files \(.ZIP format\), which can be used as a backup, or for re-importing \(see [Exporting data set](/TA_Help/Topics/Projects_and_tests_dataset_export.html), option \#2, for details\).
