...
2.  In the New Data Set dialog box, enter the details for the data set:
    ...
    -   **Data source file**: \(Optional\) Click ![](/images/TA_Help/Images/btn.browse-ellipsis.01.png) to browse to the external source file to import its data into the data set \(see [Importing a source file into a data set](/user-guide/projects-and-project-items/project-items/data-sets/importing-a-source-file-into-a-data-set) for details regarding importing a single external source file into a data set\).
3.  Click **Create**.
...
5.  Enter the test data. Beginning with the leftmost column, enter a [row](/automation-guide/action-based-testing-language/built-in-actions/test-support-actions/data-sets/row) action, followed by the data values for that row.
