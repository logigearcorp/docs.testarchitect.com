...
description: "Using anchor and anchor pos properties within variable-based dynamic identifiers."
weight: 2
aliases: 
...
As before, we can use the **div** element with `id="national_section"` as an anchor for the headline elements. In that example, we also used mappings from the interface entity to identify the link containing each headline. Because these interface elements are hard-coded and unchangeable during runtime, we refer to them as static identifiers. What we need instead in this case is a [dynamic identifier](/user-guide/interface-definitions/dynamic-identifiers), one which can be varied during the run to identify each headline-containing link element in sequence.
...
7.  Now [execute](/user-guide/test-execution/methods-of-test-execution/configuring-and-running-tests-from-testarchitect-client) your test case. \(If your test module now holds two test cases, you might want to restrict execution to just this test case.\)
