...
description: "How to export your test results to an HTML file."
weight: 2
aliases: 
    - /TA_Help/Topics/Test_result_export_HTML.html
keywords: "test results, local, HTML, exporting to, exporting, test results, local, HTML"
---
...
-   **Option \#1**: Manually exporting test results to HTML.
    ...
    1.  Select the local test results that you want to export to an HTML file.
        ...
        {{<tip>}} You can select local test results in the TestArchitect explorer tree, or on the [local test result list view](/user-guide/projects-and-project-items/project-items/list-view/result-list-view/local-result-list-view).
    ...
    2.  Invoke the Save Result\(s\) To File dialog box by doing one of the following:
        ...
        -   Double-click the selection, and then click the **Save to file** ![](/images/TA_Help/Images/Save_to_file_HTML_icon.png) icon on the main panel's toolbar.
    3.  In the Save Result\(s\) To File dialog box, choose a location to which to save the HTML file.

        ![](/images/TA_Help/Images/Save_results_to_file_dlg.png)
...
-   **Option \#2**: Directing TestArchitect to automatically export test results to HTML upon test completion.
    ...
    5.  Click ![](/images/TA_Help/Images/btn.browse-ellipsis.01.png) to choose a location to which to save the HTML file.

        ![](/images/TA_Help/Images/export_repo_result_HTML_screenshot_automatic.png)
    ...
    {{<note>}} If you opt to generate a batch file \(rather than execute the test immediately\), the following parameters are written to the generated file. \([Learn more](/user-guide/test-execution/methods-of-test-execution/executing-tests-from-the-command-line-interface/#plentry.paramters_html)\).
