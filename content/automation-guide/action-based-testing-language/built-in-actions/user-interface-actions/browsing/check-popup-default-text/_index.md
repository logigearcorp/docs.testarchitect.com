...
-   expected argument:
    ...
    -   In cases where multiple prompts satisfy the contents of message, a single call to this action can be used to check any or all of the respective default text strings. Specify as many expected arguments as there are texts you wish to verify, taking care to number them in correspondence with the specific matching prompts of interest. \(See [Example \#2](/TA_Automation/Topics/bia_get_popup_default_text.html#section.ex02).\) Note that this check action reports a PASSED result only if *every one* of the specified expected arguments matches the default text of its corresponding prompt.
