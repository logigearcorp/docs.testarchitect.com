...
-   Since this command is used to invoke a new emulator or change the emulator behavior on-the-fly, follow up by refreshing the current webpage by means of the [refresh](/automation-guide/action-based-testing-language/built-in-actions/user-interface-actions/browsing/refresh) built-in action, to ensure all new behaviors take effect. \(See example below.\)
