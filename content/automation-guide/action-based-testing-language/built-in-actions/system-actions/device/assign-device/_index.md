...
-   {{<anchor li.obtain_device_id >}}The value for the unique id argument is obtained from the [Android Instrumentation Tool](/Android/Topics/Android_Instrumentation_tool.html) dialog box \(for Android devices\), or from the [Select device](/iOS/Topics/iOS_capturing_interface.html) dialog box \(for iOS devices\).
