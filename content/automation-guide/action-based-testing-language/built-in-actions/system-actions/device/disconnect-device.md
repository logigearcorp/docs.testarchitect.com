...
description: "Description Logically disconnect the target device from the host machine. Arguments There are no arguments for this action. Valid contexts This action may be used within the following project items: ..."
weight: 8
aliases: 
    - /TA_Automation/Topics/bia_disconnect_device.html
keywords: "built-in actions, disconnect device, disconnect device (action), iOS (action), disconnect device, Android (action), disconnect a physical mobile device, physical mobile device disconnection, detach a physical mobile device"
---
...
## {{< expand >}} Applicable Systems/Platforms {{< permerlink >}} {#bia_disconnect_device__section_pvz_tbl_zcb} 
...
-   disconnect device is not used for switching between mobile devices. Instead, use the [use device](/automation-guide/action-based-testing-language/built-in-actions/system-actions/device/use-device) action, which automatically performs a logical disconnect from a connected device before connecting to the specified device.
