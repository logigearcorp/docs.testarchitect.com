...
---
# Getting started with web testing {#concept_uql_lcd_m4 .concept}
...
## Specifying the browser { .section}

To select the type of web browser to run the test in, you may specify the browser in the [use browser](bis_use_browser.html) built-in setting. For example, you can choose Mozilla Firefox to be invoked.
...
## Launching the browser and navigating to web pages { .section}

You normally launch a browser and open the web page to be tested at the beginning of a web test. You may also need to navigate to web pages in an already-running browser. You can perform these operations with the [navigate](bia_navigate.html) action.
...
|[clear popup queue](bia_clear_popup_queue.html)|Clear remaining and unhandled popups in the current queue.|
...
## Testing a web app in mobile emulation mode { .section}
...
To invoke a mobile browser emulator in Device Mode, you first need to set up the correct mobile browser profile with the [send command to browser](bia_send_command_to_browser.html) built-in action. \(See [Creating mobile browser profiles](aut_app_testing_responsive_web_Chrome_DevTools_creating_profiles.html) to learn more about JSON-based device profiles.\) The JSON string in the listing below includes the following parameters for a given mobile browser profile:
...
-   Enable touch event emulation \(enabled parameter\).
    ...
    ![](../Images/JSON_Chrome.png)

    With your JSON profile for a given device browser established, copy the JSON string into the command argument of [send command to browser](bia_send_command_to_browser.html) in your test.


**Remember:** When invoking an emulator, you are required to refresh its current page by means of the [refresh](bia_refresh.html) built-in action before proceeding with further testing. This helps ensure that the invoked emulator takes effect properly.

As an example, the following snippet of test opens the google.com page \(with [navigate](bia_navigate.html)\), invokes a mobile browser profile \(using the [send command to browser](bia_send_command_to_browser.html) built-in action\).
...
You may wish to change the environment for the emulator to customize its behaviors, after it has been launched. Again you can use the [send command to browser](bia_send_command_to_browser.html) built-in action \([learn more](aut_app_testing_responsive_web_Chrome_DevTools_launching_emulator.md#step_d2w_t3d_1w)\). For instance, you may choose to change the following behaviors:
...
-   Whenever you invoke an emulator, or change an emulator's behaviors on-the-fly, you are required to refresh its current page by means of the [refresh](bia_refresh.html) built-in action before proceeding with further testing. This helps ensure that the invoked emulator and all new behaviors take effect properly.
...
**Previous topic:**[Testing web applications - Overview](../../TA_Automation/Topics/aut_app_testing_web_apps_basics.html)

**Next topic:**[HTML5 support](../../TA_Automation/Topics/HTML5_automation.html)
