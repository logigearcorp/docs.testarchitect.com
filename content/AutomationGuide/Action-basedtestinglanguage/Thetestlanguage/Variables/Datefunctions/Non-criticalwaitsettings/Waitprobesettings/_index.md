...
---
# Wait probe settings {#timing_wait_probe_settings .concept}
...
From a functional standpoint, there is really no essential difference in the way the probe settings, window wait probe and object wait probe, are applied, compared to their critical counterparts, window wait and object wait . \(Although it is true, as specified [here](timing_sampling_logic.md#imgtbl_sampling_logic_if), that the if...exists actions do handle the [sampling logic](timing_sampling_logic.html) differently\). The primary difference is in the timing values they hold in a typical test environment. The defaults for window wait and object wait are 20 seconds each; for window wait probe and object wait probe, they’re 2 seconds.
