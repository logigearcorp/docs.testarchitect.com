...
---
# Local result list view {#result_local_list_view .concept}
...
![](../Images/Listview_local_result.png)

The following operations can be performed on the local test results list view:

## Common operations { .section}

You can find various common operations that can be performed on the local test result list view at [Common operations](Listview_common_operations.html).

**Note:** Multiple field editing operations are not applicable to the local test results list view.

## Opening result { .section}
...
## Storing local results to the repository { .section}

Local test results may be added to the repository manually or automatically. \([Learn more.](Test_result_storing.html)\)

## Opening the source test module {#section.Open_source_TM .section}
...
## Re-executing the tests {#section.Reexecute_tests .section}
...
## Opening a baseline test result { .section}

The baseline result for a test related to a selected test result can be displayed by selecting **Go to baseline result** from the context menu. \([Learn more](Test_result_baselining.md#section_kyn_rmm_bl).\)

## Comparing test results { .section}

Side-by-side comparison of test results, or side-by-side comparison between a test result and a baseline result from the same test module, enable you to track your progress over time. \(For the detailed information, see [Comparing results with baseline](Test_result_compare_to_baseline_results.html) and [Comparing results without baseline](Test_result_compare_results.html).\)
...
-   HTML files \([learn more](Test_result_export_HTML.html)\)
...
## Previewing summary of a checkpoint failure or an automation warning/error {#section.preview_checkpoint .section}
...
## Integration with third-party tools {#section.Local_listView_third_party_tools .section}
