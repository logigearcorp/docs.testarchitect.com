---
title: "Re-signing and installing TestArchitect Agent on iOS {#InstallingTAon_iOS .task}"
---

# Re-signing and installing TestArchitect Agent on iOS {#Installing_TA_on_iOS .task}
...
Ensure that the basic requirements are met before installing TestArchitect Agent onto an iOS device \(see [Prerequisites](iOS_prerequisite.md)\).
...
7.  Connect the iOS device to the test machine using a USB cable, and install a re-signed TestArchitect Agent onto the device with iTunes.
    ...
    **Tip:** Step [7](iOS_installing_TA_target_device.md#step_tww_zzq_kn) may be performed on either an macOS or Windows machine.
...
8.  You must synchronize essential TestArchitect Agent development libraries with the iOS device before TestArchitect Agent can be launched on the device.
    ...
    2.  Start Xcode on the test machine and wait at least ten seconds before disconnecting the iOS device or closing Xcode.
        ...
        **Note:** If the device is not a development device, [enable development mode](iOS_prerequisite_enable_development_mode.md) on it.

    **Important:** TestArchitect Agent libraries are purged each time the iOS device restarts or reboots. Therefore, in order to run the Agent on a rebooted iOS device, you must resynchronize the TestArchitect Agent development libraries with the device by repeating step [8](iOS_installing_TA_target_device.md#step_bmh_5rd_3n).
...
**Previous topic:**[Enabling development mode on iOS device](../../iOS/Topics/iOS_prerequisite_enable_development_mode.md)

**Next topic:**[Re-signing and installing applications on iOS](../../iOS/Topics/iOS_installing_applications.md)
